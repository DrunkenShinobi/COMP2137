...
update_netplan() {
    ...
    if grep -q "$targetIP/$targetSubmask" "$netplanConfig"; then
        echo "Netplan configuration is already set..."
    else
        echo "Setting your new netplan configuration..."
        sudo sed -i "/addresses:/a \ \ \ \ \ \ \ \ - $targetIP/$targetSubmask" "$netplanConfig"
        sudo netplan apply
